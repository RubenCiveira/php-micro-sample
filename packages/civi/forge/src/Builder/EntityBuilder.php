...
class EntityBuilder
{
    public static function generateEntityFromType(InputObjectType|ObjectType $type, bool $root, string $fqn, string $filePath): void
    {
        $fqn = str_replace('.', '\\', $fqn);
        ...
        foreach ($type->getFields() as $field) {
            $propType = self::mapGqlTypeToPhp($field);
        ...
        if (!file_exists($filePath)) {
            ...
            self::buildFromMethod($type, null, $stmts);
        ...
        if ($fromMethodRef || !in_array('from', $existingMethods)) {
            self::buildFromMethod($type, $fromMethodRef, $ast);
        }
            ...
            if ($node instanceof Namespace_) {
                foreach ($node->stmts as $stmt) {
                    if ($stmt instanceof Class_) {
                        self::reorderClassStatements($stmt);
                    }
                }
            }
        }
    ...
    public static function mapGqlTypeToPhp($field): string
    {
        $type = $field->getType();
    ...
    public static function buildFromMethod($type, $fromMethodRef, $ast)
    {
        $fromMethodBody = [];
    ...
    public static function reorderClassStatements(Class_ $class): void
    {
        $properties = [];
        $methods = [];
        $others = [];
