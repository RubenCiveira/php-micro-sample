...
class Engine
{
    ...
    public function __construct(private readonly JsonDbConfig $config, private readonly GitBackupManager $backupManager, string $baseDir = '')
    {
        $this->baseDir = $baseDir !== '' ? $baseDir : ProjectLocator::getRootPath() . '/storage';
        $this->backup();
        die("BACKUP.... {$this->config->backupRemote}");
    }
    ...
    {
        if( $this->config->backupRemote ) {
            echo "<p>INIT</h1>";
            var_dump( $this->backupManager->init() );
            echo "<p>BACKUP</h1>";
            var_dump( $this->backupManager->backup() );
            echo "<p>DIARY</h1>";
            var_dump( $this->backupManager->diary() );
        }
    }
